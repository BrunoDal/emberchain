...
type ChainSignal = { label: string; role: "source" | "target" };

function getChainSignals(cards: OwnedCard[]) {
  const signals = new Map<string, ChainSignal>();
  let preparation: { uid: string; label: string } | null = null;
  let burningSource: string | null = null;

  for (const card of cards) {
    if (card.cardId === "ember-brand") {
      preparation = { uid: card.uid, label: "+15" };
      continue;
    }
    if (card.cardId === "rage") {
      preparation = { uid: card.uid, label: card.level >= 2 ? "+70%" : "+35%" };
      continue;
    }
    if (card.cardId === "fireball" || card.cardId === "burning-edge") burningSource = card.uid;
    const definition = getCardDefinition(card.cardId);
    if (preparation && definition.tags.includes("attack")) {
      signals.set(preparation.uid, { label: `→ ${preparation.label}`, role: "source" });
      signals.set(card.uid, { label: `✦ ${preparation.label}`, role: "target" });
      preparation = null;
    } else if (burningSource && card.cardId === "ember-surge") {
      signals.set(burningSource, { label: "→ brûlure", role: "source" });
      signals.set(card.uid, { label: "✦ brûlure", role: "target" });
      burningSource = null;
    }
  }
  return signals;
...
function SortableCard({ card, index, locked, active, chain, dragging = false, onTap, onDragStart, onDragMove, onDragEnd, onDragCancel }: { card: OwnedCard; index: number; locked: boolean; active: boolean; chain?: ChainSignal; dragging?: boolean; onTap: () => void; onDragStart: (event: ReactPointerEvent<HTMLButtonElement>, uid: string) => void; onDragMove: (event: ReactPointerEvent<HTMLButtonElement>) => void; onDragEnd: (event: ReactPointerEvent<HTMLButtonElement>) => void; onDragCancel: (event: ReactPointerEvent<HTMLButtonElement>) => void }) {
  return <div className={`hand-slot ${active ? "is-active" : ""} ${dragging ? "is-dragging" : ""} ${chain ? `chain-${chain.role}` : ""}`} data-card-uid={card.uid} style={{ "--deal-index": index } as CSSProperties}><span className="slot-number">{String(index + 1).padStart(2, "0")}</span>{chain && <span className="chain-indicator">{chain.label}</span>}<CardTile card={card} selected={dragging} locked={locked} onTap={onTap} onPointerDown={(event) => onDragStart(event, card.uid)} onPointerMove={onDragMove} onPointerUp={onDragEnd} onPointerCancel={onDragCancel} /></div>;
}
...
function combatEventDelay(event: CombatEvent | null, fastMode: boolean, motionReduced: boolean) {
  if (motionReduced) return 420;
  const normalDelay = !event ? 500
    : event.type === "enemyWindup" ? 1650
      : event.type === "cardPlayed" ? 1050
        : event.type === "damage" && event.tags?.includes("enemy-attack") ? 1000
          : event.type === "damage" ? 800
            : event.type === "enemyDefeated" || event.type === "heroDefeated" ? 1150
              : event.type === "shield" || event.type === "heal" ? 760
                : event.type === "statusApplied" || event.type === "combo" ? 680
                  : 620;
  return fastMode ? Math.max(280, Math.round(normalDelay / 2)) : normalDelay;
}
...
function App() {
  ...
  useEffect(() => { if (!isCombat) return; const timer = window.setTimeout(() => dispatch({ type: "ADVANCE_COMBAT" }), combatEventDelay(state.lastEvent, fastMode, motionReduced)); return () => window.clearTimeout(timer); }, [fastMode, isCombat, motionReduced, state.combatCursor, state.lastEvent]);
